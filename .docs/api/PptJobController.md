...
| `confirmationPayload` | agent 给前端展示的确认信息 |
| `events` | 任务事件历史 |
| `artifactReady` | 是否已有可下载 PPTX 产物 |
| `downloadUrl` | 完成后返回下载地址，未完成时为空 |
...
5. 任务进入 `WAITING_CONFIRMATION`，前端通过查询或 SSE 获取 `confirmationPayload`；其中包含 `replyId`、待恢复的 `toolCalls`、推荐方案和 agent 模式说明。
